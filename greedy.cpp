...
void followString(std::string path){

    for (int i = 0; i < path.length(); i++){
        if (path[i] == 'F'){ 
            API::moveForward(1);
            std::cerr<<"Step Forward"<<std::endl;
        }
        else if (path[i] == 'R') {
            API::turnRight();
            API::moveForward(1);
...
int main(int argc, char *argv[]) {
    std::string b2s = "FFFFRLRFLRFFFFRFFRLFRLRFF";
    followString(b2s);
    API::turnRight();
    ...
    std::string opt = "RFFLRLFRLFFLFFFFLFRLLRRLLFFLRLFFLFL";

    followString(std::reverse(opt.begin(), opt.end()));
