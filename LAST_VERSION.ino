...
void setup() {
  ...
  // motors_straight(true,66,76,25); //difference of 42.5,49 or 
  delay(200);
...
// void hardcoded_turn_90(bool R_L){
//   motors_stop(2);
//   delay(500);
//   if()
//     Turn(R_L,90,83);
...
//   motors_stop(2);
// }
int TURN_90_DEGREES_COUNTS = 650;
void encoder_turnLeft90() {
  int encoder1_before = encoderValue;
  int encoder2_before = encoderValue2;
  while (abs(encoderValue - encoder1_before) < TURN_90_DEGREES_COUNTS && abs(encoderValue2 - encoder2_before) < TURN_90_DEGREES_COUNTS) {
    readEncoders();
...
void turn_90(bool R_L){
  ...
  while(true){
    ...
    if (cm[1] > 4) {  
      ...
      motors_straight(false,65,70,25);
    }
  }
  delay(500);
  while(true){
...
if(first_time){
  motors_straight(false,speed_right/2,speed_left/2,25);
  delay(50);
  first_time = false;
}
  motors_straight(false,speed_right,speed_left,25);
  // delay(50);
}
void Logan_lanekeep(){
// delay(100);
...
{
        if (R_L){
            setSpeed(70,PWMright1); //83
            setSpeed(0,PWMright2);
            setSpeed(0,PWMleft1);
            setSpeed(76,PWMleft2); //96
        }
        else{
            setSpeed(0,PWMright1);
            setSpeed(70,PWMright2); //83
            setSpeed(76,PWMleft1); //96
            setSpeed(0,PWMleft2);
        }
