...
void setup() {
  ...
  motors_straight(true,90,90,25);
  // motors_straight(true,speed_right/2,speed_left/2,25); //difference of 42.5,49 or 
  // delay(200);

}
double toPWM(double spd){
  return 2.99 + (-0.0423*spd) + ((.000443 *(spd*spd)));
}
...
void updateMotorSpeeds(double desiredSpeed, double dt) {
  ...
  double leftTick = leftMotorPID.tick(currentSpeedLeft, desiredSpeed, dt);
  double rightTick = rightMotorPID.tick(currentSpeedRight, desiredSpeed, dt);
  
  double leftMotorOutput = toPWM(leftTick);
  double rightMotorOutput = toPWM(rightTick);
...
double getspeedLeft(){ //right is 1 left is 2
  ...
  if (currentMillis2 - previousMillis2 >= interval) {
    ...
    speed2 = (encoderValueEND2-encoderValueSTART2);
...
return encoderValue2; // Example calculation (replace with actual logic)
}

int getRightEncoderSpeed() {
// Implement encoder reading and speed calculation for right motor
return encoderValue; // Example calculation (replace with actual logic)
}
